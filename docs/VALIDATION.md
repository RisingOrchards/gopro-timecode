...
- A configuration QR scan on original GoPro firmware returned “Timecode not synced” and applied no settings, as confirmed by the project owner. The settings page, README and guide state the GoPro Labs firmware requirement explicitly and provide a manual-settings workflow for stock firmware. The owner subsequently reported “I updated the firmware and everything works perfectly,” establishing field-tested use after the update. Exact firmware and per-preset results were not supplied; see the [hardware report](HARDWARE_VALIDATION.md).

- Capture target defaults to 4K 16:9 / 24 fps. Cinema, Run & Gun and Custom inherit it on all three models. Cinema and Run & Gun default to 5500K, a 180° shutter and Auto ISO up to 800, with HyperSmooth Off and On respectively. Slow Motion keeps 4K60 and High Frame Rate keeps 4K120 with visible override feedback. All supported model/target combinations were exercised for the three target-following presets. ILS lens confirmation follows GoPro’s current supported-lens guidance, including its specifically profiled fisheyes.

- Streaming / Live defaults to 1080p 16:9, target rate up to 60 fps, Natural color, 10-bit, 5500K, 180° shutter, Auto ISO up to 800 and stabilization Off. All three models and every target rate were exercised, including explicit 60 fps overrides for higher targets. Browser checks cover persistence, Photo/Video transitions, editable color, QR invalidation and desktop/mobile layout. HDMI setup remains manual; successful HDMI capture has not been verified on hardware. The source references and setup steps are in the guide.

- All **46** Node tests passed: 31 timecode/lifecycle/QR tests and 15 settings tests. Settings checks cover explicit preset command fixtures, target inheritance and overrides, model/resolution/rate limits, GP-Log2 depth, ISO/shutter/EV rules, ILS lens confirmation, Photo isolation, per-field command changes, versioned state migration, malformed stored state, command injection and QR quiet zones.
...
These checks verify software output. Camera Settings QR is also field tested after a firmware update; the earlier production-shoot report applies to Timecode QR. See [camera-settings research and omissions](CAMERA_SETTINGS.md) for the documented scope.
